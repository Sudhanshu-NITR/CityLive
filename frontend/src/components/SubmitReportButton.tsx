...
export default function SubmitReportButton({ onClick }: { onClick: () => void }) {
    return (
        <button
            onClick={onClick}
            className="group relative w-full overflow-hidden rounded-2xl p-px transition-all duration-300 hover:scale-[1.02] hover:shadow-[0_0_40px_-10px_rgba(59,130,246,0.6)]"
        >
            {/* Animated Gradient Border */}
            <span className="absolute inset-0 bg-linear-to-r from-blue-600 via-cyan-400 to-blue-600 opacity-70 group-hover:opacity-100 transition-opacity duration-500" />
